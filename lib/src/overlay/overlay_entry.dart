...
class ZoOverlayEntry extends ChangeNotifier {
  ...
  ///
  /// 设置为 [alwaysOnTop] 的层会禁用 hover / focus / tapAwayClosable 等功能
  bool alwaysOnTop;
  ...
  /// 执行一组更改操作, 并且只触发一次更新
  ///
  /// 可以将 [notify] 设置为 false 来阻止更新
  void actions(VoidCallback callback, [bool notify = true]) {
    _lockNotify = true;
    callback();
    _lockNotify = false;
  ...
  /// 子类可通过重写此方法来定义层的内容, 默认实现会直接调用 [builder] 方法
  @protected
  ...
  /// 组id, 用于 [tapAwayClosable] 实现, 可以将其添加到其他 [TapRegion] 来防止 [tapAwayClosable]
  /// 触发关闭
